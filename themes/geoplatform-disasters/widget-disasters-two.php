 ...
 * @since 2.0.0
 */
class Geopccb_Disasters_Two_Widget extends WP_Widget {

  // Constructor. Simple.
	function __construct() {
		parent::__construct(
			'geopccb_disasters_two_widget', // Base ID
			esc_html__( 'GeoPlatform Disasters Two', 'geoplatform-ccb' ), // Name
			array( 'description' => esc_html__( 'GeoPlatform Disasters widget two for list pages. Takes a category as input and displays its content, along with their featured images, in a tile format.', 'geoplatform-ccb' ), 'customize_selective_refresh' => true) // Args
		);
	}
...
function geopccb_register_disasters_two_widget() {
		register_widget( 'Geopccb_Disasters_Two_Widget' );
}
add_action( 'widgets_init', 'geopccb_register_disasters_two_widget' );
